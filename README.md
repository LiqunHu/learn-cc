# Learn Claude Code -- Harness Engineering for Real Agents (JavaScript)
...
async function agent_loop(messages) {
  while (true) {
    ...
    for (const tool_call of assistant.tool_calls) {
      ...
      const output = await TOOL_HANDLERS[name](args)
      messages.push({
        role: 'tool',
...
This JavaScript edition uses an OpenAI-compatible Chat Completions API (`tool_calls` / `role: "tool"`). The harness ideas are the same whether the provider is Anthropic, OpenAI, or another compatible endpoint.

---

## Version Status

This repository is a **JavaScript (Node.js / `.mjs`) port** of the harness engineering curriculum.

- **Current track: root-level `s01-s20`**
  Each chapter is intended to include a runnable `code.mjs` and diagrams where needed. Chapters land progressively -- start with what exists today (`s01`, `s02`, ...) and grow toward the full 20-lesson track.
- Upstream Python reference: [shareAI-lab/learn-claude-code](https://github.com/shareAI-lab/learn-claude-code)

If you are starting now, begin at `s01_agent_loop/` and work forward in order.
...
>
> **s13** &nbsp; *"Slow ops go background, agent keeps thinking"* &mdash; background workers run commands; notifications inject on completion
>
...
|---|---|---|
| [s01](./s01_agent_loop/) | Agent Loop | `messages` / `while (true)` / `tool_calls` |
...
| [s13](./s13_background_tasks/) | Background Tasks | async workers / notification queue |
...
```
s02_tool_use/
  code.mjs               # standalone runnable implementation
  images/                # SVG diagrams (where needed)
```

Read the chapter code and diagrams in order. Each chapter assumes you've absorbed the previous ones and builds one more harness mechanism on the same agent loop.
...
cd learn-cc
npm install
cp .env_example .env   # configure OPENAI_API_KEY / OPENAI_API_BASE_URL / OPENAI_MODEL

node s01_agent_loop/code.mjs   # Start here -- one loop + bash
node s02_tool_use/code.mjs     # Multiple tools + dispatch map
# later chapters:
# node s08_context_compact/code.mjs
# node s20_comprehensive/code.mjs
```

Environment variables (see `.env_example`):

```
OPENAI_API_BASE_URL=   # OpenAI-compatible chat completions endpoint
OPENAI_API_KEY=
...
```
learn-cc/
  package.json             # Node.js dependencies (dotenv, gaxios, glob, ...)
  .env_example             # API config template
  s01_agent_loop/
    code.mjs               #   standalone runnable code
    images/                #   SVG diagrams
  s02_tool_use/
    code.mjs
    images/
  ...
  s19_mcp_plugin/
  s20_comprehensive/       # endpoint chapter (planned)
```
